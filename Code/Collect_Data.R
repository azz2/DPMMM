Collect_Data <- function(triplet, bin_width, Triplet_meta){
  ...
  trials <- tryCatch({trials <- read.table(url(paste(VC.url, 
                                                     fname,
                                                     ".txt",
                                                     sep="") ), 
                                           sep="\t" )
  ...
  spiketimes <- tryCatch({spiketimes <- read.table(url(paste(VC.url,
                                                             fname, 
                                                             "_cell", 
                                                             cell, 
                                                             "_spiketimes.txt",
                                                             sep="") ), 
                                                   sep="\t" )
  spiketimes[,-dim(spiketimes)[2]]},
  error = function(e) {
    url = paste(JA.url, 
                fname,
                "_spiketimes.txt",sep="")
    spiketimes <- read.table(url(paste(JA.url,
                                       fname, 
                                       "_spiketimes.txt",sep="") ), 
                                 sep="" )
    return(spiketimes)})
  ...
  AB = Bincounts(trials, spiketimes, frq = c(freq, 742), 
                 pos = c(pos, -144/pos), on.reward = TRUE, 
                 start.time = 0, end.time = 1000, 
                 bw = bin_width, target = c(25, 150, 800), 
                 match.level = FALSE, AB.eqlevel = FALSE, 
                 go.by.soff = TRUE, n.iter = 1e3, plot = FALSE,
                 faux.dual.mix = FALSE, faux.dual.int = FALSE, 
                 faux.alpha = 0.5, faux.dual.swi = FALSE, 
